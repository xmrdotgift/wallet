    ...
    export default {
        ...
        computed: {
            ...
            canDeposit() {
                ...
                return this.currentCard === "deposit" && action !== "loading" && action !== "error"
            },

            canShare() {
                const action = this.walletStatus.action
                return this.currentCard === "share" && action !== "loading" && action !== "error"
            },

            canRedeem() {
                const status = this.walletStatus
                return this.currentCard === "redeem" && !status.empty && status.unlocked
            },

            canNavigate() {
                return !this.canDeposit && !this.canShare && !this.canRedeem
            },
        ...
        watch: {
            ...
            appParams: {
                handler(val) {
                    ...
                    if (s !== "") {
                        const pushState = (this.currentCard === "" && card !== "") || (this.currentCard !== card && card !== "")
